...
if __name__ == '__main__':
    ...
    GM_err = []
    S_err = []
    samples = [1000, 5000, 10000,15000, 20000,25000, 30000,35000, 40000,45000, 50000]
    for jj in range(len(samples)):
        ...
        Y_0 = Y_predict[0]
        S_err.append(np.mean(np.argmax(y_test,axis=1)==np.argmax(Y_0,axis=1)))
        fusion_err.append(np.mean(np.argmax(y_test,axis=1)==np.argmax(Y_fusion,axis=1)))
        AM_err.append(np.mean(np.argmax(y_test,axis=1)==np.argmax(Y_AM,axis=1)))
        GM_err.append(np.mean(np.argmax(y_test,axis=1)==np.argmax(Y_GM,axis=1)))
    
    plt.plot(samples,fusion_err,label = 'Chernoff fusion' )
    plt.plot(samples,AM_err,label = 'Average mean' )
    plt.plot(samples,GM_err,label = 'Geometrical mean' )
    plt.plot(samples,S_err,label = 'Single NN' )
